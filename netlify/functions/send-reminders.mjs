...
export default async () => {
  ...
  if (!subscription || !Array.isArray(events)) {
    console.log("Brak subskrypcji albo wydarzeń.");
    return;
  }
  ...
  if (schoolTomorrow.length > 0) {
    ...
    for (const hour of schoolHours) {
      ...
      try {
        console.log("Próba wysłania szkolnego Push:", schoolKey);
        ...
        console.log("Szkolny Push wysłany. Status:", result.statusCode);

        sentReminders[schoolKey] = new Date().toISOString();

        await store.setJSON("sent-reminders", sentReminders);
      } catch (error) {
        console.error("❌ BŁĄD SZKOLNEGO PUSH:", error);
  ...
  for (const event of events) {
    ...
    const eventTime = new Date(
      `${event.date}T${event.time}:00+02:00`
    );
    ...
    if (sentReminders[reminderKey]) {
      console.log("Przypomnienie już wysłane:", reminderKey);
      continue;
    }
    ...
    const body =
      event.title || "Masz zaplanowane wydarzenie.";
